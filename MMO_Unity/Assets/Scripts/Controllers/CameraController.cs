...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        cam.position = _player.transform.position + Quaternion.identity * pivotOffset + Quaternion.identity * camOffset;
        ...
        angleH = _player.transform.eulerAngles.y;
    ...
    void LateUpdate()
    {
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            Vector3 baseTempPosition = _player.transform.position + camYRotation * targetPivotOffset;
            ...
            cam.position = _player.transform.position + camYRotation * smoothPivotOffset + aimRotation * smoothCamOffset;
            return;
        }
        else
        {
        ...
        //        transform.position = _player.transform.position + new Vector3(0.0f,1.0f,0.0f) + _delta.normalized * dist;
        //    }
        //    else
        //    {
    ...
    bool ViewingPosCheck(Vector3 checkPos)
    {
        ...
        Vector3 target = _player.transform.position + pivotOffset;
        ...
        if (Physics.SphereCast(checkPos, 0.2f, direction, out RaycastHit hit, direction.magnitude))
        {
            ...
            if (hit.transform != _player.transform && !hit.transform.GetComponent<Collider>().isTrigger)
            {
    ...
    bool ReverseViewingPosCheck(Vector3 checkPos)
    {
        ...
        Vector3 origin = _player.transform.position + pivotOffset;
        Vector3 direction = checkPos - origin;
        if (Physics.SphereCast(origin, 0.2f, direction, out RaycastHit hit, direction.magnitude))
        {
            if (hit.transform != _player.transform && hit.transform != transform && !hit.transform.GetComponent<Collider>().isTrigger)
            {
                return false;
            }
        }
